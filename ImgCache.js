...
(function ($) {
    ...
    ImgCache.init = function (success_callback, error_callback) {
        ...
        var _fail = function (error) {
            ImgCache.overridables.log('Failed to initialise LocalFileSystem - ' + error.name + ' - ' + error.message, LOG_LEVEL_ERROR);
            if (error_callback) { error_callback(); }
        };
        if (Helpers.isCordova()) {
            // PHONEGAP
    ...
    ImgCache.cacheFile = function (img_src, success_callback, error_callback, on_progress) {
        ...
        fileTransfer.download(
            ...
            function (entry) {
                ...
                if (success_callback) {
                    success_callback(img_src, entry);
                }
            },
            function (error) {
    ...
    // Expose the helpers to the world
